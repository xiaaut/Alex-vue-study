...
export default {
  props: ['selectedId', 'onCloseMovie', 'onAddWatched', 'watched'],
  ...
  methods: {
    ...
    handleAdd() {
      ...
      const watchedMovie = { id, img, nm, rt, sc };
      localStorage.setItem('watched-movie-list', JSON.stringify([...this.watched, watchedMovie]));

      this.onAddWatched({ id, img, nm, rt, sc });
    },
  },
  ...
  watch: {
    async selectedId() {
      this.isLoading = true;
      const response = await fetch(`/movieDetail?movieId=${this.selectedId}`);
